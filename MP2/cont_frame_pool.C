                             ...
                             unsigned long _n_info_frames)
{
  ...
  */
    ...
    */
    memset(bitmap,0xFF,(n_frames-1)/8 + 1);
    /*
...
{
    if(n_free_frames <=0 || _n_frames > n_free_frames) return 0;
    unsigned int i = 0;
    const char mask = 0xC0;
    while(i <= 2*(n_frames - _n_frames))
    {
      unsigned int j = i; unsigned int valid = 0;
      while(valid < _n_frames)
      {
        char m = mask >> (j%4);
        if((bitmap[i/4] & m) ==  m) valid++;
        else break;
        j+=2;
      }
      if(valid == _n_frames)
      {
        const char mask = 0x3F; unsigned int j = i;
        while(valid>0)
        {
          char m = mask>>(j%4) | mask << (8 - (j%4));
          bitmap[i/4] &= m;
          valid--;
        }
        //mark HEAD
...
void ContFramePool::release_frames(unsigned long _first_frame_no)
{
    ...
    if(ptr!=NULL)
    {
      unsigned int offset = _first_frame_no - ptr->base_frame_no;
      unsigned int i = offset/4;
      unsigned int j = offset%4;
      char mask = 0xC0 >> j;
      bitmap[i] |= mask;
    }
}
